...
export default async function ColetaFuraFuraDetailPage({
  ...
  const [{ data: pagamentos }, { data: empresa }, { data: itemVisita }] = await Promise.all([
    supabase
  ...
  const editarHref = (() => {
    ...
    return `/coletas/nova/fura-fura?${params.toString()}`;
  })();
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {coleta.observacao && (
        <div className="glass-card p-6">
          <p className="text-xs text-at-muted mb-1">Observação</p>
          <p className="text-sm text-at-primary/85">{coleta.observacao}</p>
        </div>
      )}
        ...
        <div className="glass-card p-6 space-y-2">
          ...
          {pagamentos!.map((p) => (
            <div key={p.id} className="flex justify-between gap-4 text-sm">
              <div>
                <span className="text-at-muted">{formatDateTime(p.created_at)}</span>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {coleta.latitude && coleta.longitude && (
          <span className="inline-flex items-center gap-1 text-xs text-at-muted px-2 py-2">
