...
const TITULO_HAVER = "Haver do ponto (fura-fura)";

export async function registrarHaverFuraFura(
  ...
  if (opts.registrarFinanceiro) {
    ...
    await supabase.from("financeiro").insert({
      empresa_id: opts.empresaId,
      tipo: "entrada",
      categoria: "Haver fura-fura",
      valor,
      descricao: detalhe
        ? `Haver ${opts.pontoNome} — ${detalhe}`
        : `Haver ${opts.pontoNome}`,
...
/** Rateia valor entre Pix e dinheiro (Pix primeiro). Atualiza os saldos restantes. */
export function splitExcedentePagamento(
  valor: number,
  pixRestante: { v: number },
  dinheiroRestante: { v: number }
): { pix: number; dinheiro: number } {
  const pixChunk = Math.min(pixRestante.v, valor);
  pixRestante.v = Math.round((pixRestante.v - pixChunk) * 100) / 100;
  const restante = Math.round((valor - pixChunk) * 100) / 100;
  const dinheiroChunk = Math.min(dinheiroRestante.v, restante);
  dinheiroRestante.v = Math.round((dinheiroRestante.v - dinheiroChunk) * 100) / 100;
  return { pix: pixChunk, dinheiro: dinheiroChunk };
}
