...
export async function getDiversaoDashboardStats(
  ...
  const [
    ...
    supabase
      .from("coletas")
      .select(
        "valor_bruto, lucro_real, valor_liquido, valor_a_receber, valor_pago_recebido, entrada_periodo, ponto_id, created_at"
      )
    ...
    supabase
      .from("coletas")
      .select("valor_bruto, lucro_real, valor_liquido, entrada_periodo")
      .eq("empresa_id", empresaId)
      .eq("nicho_modulo", NICHO_MODULO_DIVERSAO)
