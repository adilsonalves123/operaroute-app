              ...
              <div key={p.id} className="glass-card overflow-hidden">
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="truncate text-lg font-semibold text-white">{p.titulo}</p>
                      <AlertBadge
                      ...
                      {p.status === "resolvida" && (
                        <AlertBadge className="text-[11px] px-2.5 py-1" variant="success">Resolvida</AlertBadge>
                      )}
                    </div>
                    <p className="mt-1 truncate text-sm text-[var(--shell-text-muted)]">{contextoLabel}</p>
                  </div>
                ...
                {isOpen && (
                  <div className="border-t border-slate-800 px-4 pb-4 pt-3 space-y-4">
                    ...
                    {p.descricao ? (
                      <details
                        className="pendencias-historico group rounded-lg border"
                        style={{
                          background: "var(--pendencia-pay-bg)",
                          borderColor: "var(--pendencia-pay-border)",
                        }}
                      >
                    ...
                    {!editando && !isFura && (p.status === "aberta" || mostrarTodas) && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-600 px-3.5 py-2 text-sm font-medium text-slate-300 hover:bg-slate-800 disabled:opacity-50"
                      >
                    ...
                    {p.status === "aberta" && !editando && (
                      <>
                        ...
                        <div
                          className="pendencias-pagamento space-y-3 rounded-lg border p-4"
                          style={{
                            background: "var(--pendencia-pay-bg)",
                            borderColor: "var(--pendencia-pay-border)",
                          }}
                        >
                          ...
                          <div className="space-y-1.5">
                            <label className="block text-sm font-medium text-[var(--shell-text)]">
                              {valorLabelPix}
                            </label>
                            <input
                              inputMode="decimal"
                        ...
                        {form.erro && <p className="text-sm text-red-400">{form.erro}</p>}
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          ...
                          <button
                            ...
                            disabled={loadingId === p.id}
                            onClick={() => baixar(p.id)}
                            className="inline-flex items-center gap-1.5 rounded-lg border px-3.5 py-2 text-sm font-semibold disabled:opacity-50"
                            style={{
                              color: "var(--pendencia-baixa)",
                              borderColor: "var(--pendencia-baixa)",
                              background: "var(--pendencia-baixa-bg)",
                            }}
                          >
