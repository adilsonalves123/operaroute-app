...
import type { CalculoColetaUrsinho } from "@/lib/nichos/ursinho";
...
export function ColetaUrsinhoResumo({
  calculo,
  className,
  pendenciaPonto,
  haverSaldo = 0,
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className="rounded-xl border border-pink-500/25 bg-gradient-to-b from-pink-500/10 to-transparent p-4">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-pink-300/80">
        ...
        <p className="mt-1 text-3xl font-bold tabular-nums text-pink-200">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
        ...
        <span className="text-right tabular-nums text-rose-300/90">{formatCurrency(calculo.custoBrindes)}</span>
      </div>
      ...
      {recebimento && mostrandoPagamento ? (
        ...
        <div className="space-y-3">
          {finalizarSemPagar ? (
            <div className="rounded-lg border border-pink-500/25 bg-pink-500/5 px-3 py-3 text-xs text-slate-400">
              Fecha a visita sem cobrar agora. O saldo fica{" "}
              <strong className="text-pink-300">pendente</strong>.
            </div>
          ) : (
            <>
              <ColetaRecebimentoFields
