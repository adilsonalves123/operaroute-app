...
export function DonoEmpresasClient({
  email,
  saudeInicial = "todos",
  qInicial = "",
}: {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/dono/empresas?${params}`);
        ...
        <table className="w-full min-w-[720px] text-left text-[13px]">
          <thead>
            <tr className="border-b border-white/[0.06] text-[10px] uppercase tracking-[0.14em] text-slate-500">
              ...
              <th className="px-4 py-3 font-medium text-right">MRR pago</th>
            </tr>
          </thead>
              ...
              <tr key={t.id} className="transition hover:bg-white/[0.015]">
                ...
                <td className="px-4 py-3 text-right tabular-nums text-[#e8d5b0]">
                  {t.pagamento_confirmado
                    ? money(t.mrr_pago || t.mrr_estimado)
